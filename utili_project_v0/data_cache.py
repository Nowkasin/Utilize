...
def _build_mssql_odbc_str(
    server: str, db: str, user: str, password: str, driver: str
) -> str:
  return (
      "DRIVER={" + driver + "};"
      f"SERVER={server};"
...
def build_bme_map():
  ...
  # 👇 ตัด AE Title ที่ถือว่าไม่ใช่ค่าใช้งานจริง เช่น None / NaN / NULL / NA
  invalid_ae = df["ae_title_clean"].str.lower().isin(["none", "nan", "null", "na"])

  mask = (
      df["ae_title_clean"].ne("")      # ไม่ใช่ string ว่าง
      & ~invalid_ae                    # ไม่ใช่ none/nan/null/na
      & df["capEx"].notna()
      & (df["capEx"] > 0)
      & df["depYears"].notna()
      & (df["depYears"] > 0)
...
def get_lookup_maps():
  ...
  if GLOBAL_CACHE["bme_map"] is None:
    ...
    # สูตร year_month แบบที่คุณเขียน: YYYY-MM จาก exam_date
    year_month_expr = (
